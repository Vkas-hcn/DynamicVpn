...
class CKNZ : BroadcastReceiver() {
    companion object {
        private val componentName by lazy { ComponentName(app, CKNZ::class.java) }
        var enabled: Boolean
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val doStart = when (intent.action) {
            ...
            else -> DataStore.directBootAware || Build.VERSION.SDK_INT >= 24 && CKOJ.user.isUserUnlocked
        }
        if (doStart) CKOJ.startService()
    }
}
