...
class CKNY : Service(), CoroutineScope {
    companion object {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (worker == null) {
            ...
            worker = launch {
                val urls = CKOS.instance.urls
                val notification = NotificationCompat.Builder(this@CKNY, NOTIFICATION_CHANNEL).apply {
                    color = ContextCompat.getColor(this@CKNY, R.color.material_primary_500)
                    priority = NotificationCompat.PRIORITY_LOW
                    addAction(NotificationCompat.Action.Builder(
                            R.drawable.ic_navigation_close,
                            getText(R.string.stop),
                            PendingIntent.getBroadcast(this@CKNY, 0,
                ...
                CKOJ.notification.notify(NOTIFICATION_ID, notification.build())
                counter = 0
                val workers = urls.asIterable().map { url -> fetchJsonAsync(url, urls.size(), notification) }
                try {
                    val localJsons = workers.awaitAll()
                    withContext(Dispatchers.Main) {
                        CKOJ.notification.notify(NOTIFICATION_ID, notification.apply {
                    ...
                    GlobalScope.launch(Dispatchers.Main) {
                        CKOJ.notification.cancel(NOTIFICATION_ID)
                        idle.value = true
                    }
                    check(worker != null)
                    worker = null
    ...
    private fun fetchJsonAsync(url: URL, max: Int, notification: NotificationCompat.Builder) = async(Dispatchers.IO) {
        ...
        try {
            ...
            launch(Dispatchers.Main) {
                Toast.makeText(this@CKNY, e.readableMessage, Toast.LENGTH_LONG).show()
            }
