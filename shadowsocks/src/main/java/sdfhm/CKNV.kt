...
class CKNV : BaseVpnService(), CKON.Interface {
    companion object {
...
//                            OsConstants.EPERM, OsConstants.EACCES, OsConstants.ENONET -> Timber.d(e)
    ...
    inner class NullConnectionException : NullPointerException(), CKON.ExpectedException {
    ...
    override fun onBind(intent: Intent) = when (intent.action) {
        ...
        else -> super<CKON.Interface>.onBind(intent)
    }
...
//        }
    ...
    private suspend fun sendFd(fd: FileDescriptor) {
        var tries = 0
        val path = File(CKOJ.deviceStorage.noBackupFilesDir, "sock_path").absolutePath
        while (true) try {
            delay(50L shl tries)
