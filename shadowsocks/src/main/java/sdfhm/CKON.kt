...
object CKON {
    enum class State(val canStop: Boolean = false) {
        /**
         * Idle state is only used by UI and will never be returned by CKON.
         */
        Idle,
        Connecting(true),
        Connected(true),
    ...
    class Data internal constructor(private val service: Interface) {
        ...
        var processes: CKOL? = null
        var proxy: CKOK? = null
        var udpFallback: CKOK? = null
        var localDns: CKOQ? = null

        var notification: CKOM? = null
    ...
    interface Interface {
        val data: Data
        val tag: String
        fun createNotification(profileName: String): CKOM
        ...
        suspend fun startProcesses() {
            val context = if (Build.VERSION.SDK_INT < 24 || CKOJ.user.isUserUnlocked) app else CKOJ.deviceStorage
            val configRoot = context.noBackupFilesDir
            val udpFallback = data.udpFallback
            data.proxy!!.start(this,
                    File(CKOJ.deviceStorage.noBackupFilesDir, "stat_main"),
            ...
            udpFallback?.start(this,
                    File(CKOJ.deviceStorage.noBackupFilesDir, "stat_udp"),
                    File(configRoot, CONFIG_FILE_UDP),
                    "udp_only", false)
            data.localDns = CKOQ(this::rawResolver).apply { start() }
        }
...
//                Firebase.analytics.logEvent("stop") { param(FirebaseAnalytics.Param.METHOD, tag) }
        ...
        fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            ...
            val profile = CKOJ.currentProfile
            Log.i(CKON::class.simpleName, "onStartCommand: use profile[id = ${profile?.id}], host = ${profile?.host}, name = ${profile?.name}")
            this as Context
            if (profile == null) {
                Log.i(CKON::class.simpleName, "onStartCommand: no available profile, stop runner automatically")
            ...
            try {
                data.proxy = CKOK(profile)
                data.udpFallback = null //if (fallback == null) null else CKOK(fallback, profile.route)
            ...
            CKNZ.enabled = DataStore.persistAcrossReboot
            if (!data.closeReceiverRegistered) {
...
//            Firebase.analytics.logEvent("start") { param(FirebaseAnalytics.Param.METHOD, tag) }
            data.connectingJob = GlobalScope.launch(Dispatchers.Main) {
                try {
                    CKOR.killAll()    // clean up old processes
                    preInit()
