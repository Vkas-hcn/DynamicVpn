 ...
 */
class CKOK(val profile: Profile, private val route: String = profile.route) {
    init {
...
//        }
     ...
     */
    fun start(service: CKON.Interface, stat: File, configFile: File, mode: String, dnsRelay: Boolean = true) {
        // setup traffic monitor path
        trafficMonitor = CKOO(stat)
        ...
        config.put("locals", JSONArray().apply {
            ...
            if (dnsRelay) try {
                ...
                throw CKON.ExpectedExceptionWrapper(e)
            }.let { dns ->
                put(JSONObject().apply {
                    put("local_address", DataStore.listenAddress)
        ...
        val cmd = arrayListOf(
                File((service as Context).applicationInfo.nativeLibraryDir, CKOR.SS_LOCAL).absolutePath,
