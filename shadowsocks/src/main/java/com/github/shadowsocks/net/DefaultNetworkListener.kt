...
object DefaultNetworkListener {
    ...
    suspend fun get() = if (fallback) @TargetApi(23) {
        CKOJ.connectivity.activeNetwork ?: throw UnknownHostException() // failed to listen, return current if available
...
//                CKOJ.connectivity.registerBestMatchingNetworkCallback(request, Callback, mainHandler)
//            }
            in 28 until 31 -> @TargetApi(28) {  // we want REQUEST here instead of LISTEN
                CKOJ.connectivity.requestNetwork(request, Callback, mainHandler)
            }
            in 26 until 28 -> @TargetApi(26) {
                CKOJ.connectivity.registerDefaultNetworkCallback(Callback, mainHandler)
            }
            in 24 until 26 -> @TargetApi(24) {
                CKOJ.connectivity.registerDefaultNetworkCallback(Callback)
            }
            else -> try {
                fallback = false
                CKOJ.connectivity.requestNetwork(request, Callback)
            } catch (e: RuntimeException) {
                fallback = true     // known bug on API 23: https://stackoverflow.com/a/33509180/2245107
            }
        }
    }
    private fun unregister() = CKOJ.connectivity.unregisterNetworkCallback(Callback)
}
