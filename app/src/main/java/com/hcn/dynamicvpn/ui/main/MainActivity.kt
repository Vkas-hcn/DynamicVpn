...
import com.hcn.dynamicvpn.ui.result.ResultActivity
import com.hcn.dynamicvpn.ui.web.WebActivity
...
class MainActivity : BaseActivity<ActivityMainBinding, MainViewModel>(),
    ...
    OnPreferenceDataStoreChangeListener, LifecycleObserver, MainCall {

    override val model by viewModels<MainViewModel>()
...
//        }
        ...
        showVpnGuideDynamic()
//        App.nativeAdRefreshDynamic = true
    }
    ...
    private fun setTitleData() {
        with(binding.inMainTitle) {
            this.tvTitle.text = this@MainActivity.getString(R.string.app_name)
        ...
        model.liveJumpResultsPage.observe(this, {
            lifecycleScope.launch(Dispatchers.Main.immediate) {
                ...
                if (lifecycle.currentState == Lifecycle.State.RESUMED) {
                    launchActivityForResult(ResultActivity::class.java, 0x78, it)
                }
            }
        })
    }
...
//            BaseAdLoad.getBackInstance().advertisementLoadingDynamic(this@BG)
            bundle.putString(DataUtils.currentDynamicService, DataUtils.current_server_data_dynamic)
            launchActivityForResult(ListActivity::class.java, 0x77, bundle)
        }
    }
...
//            DynamicUtils.putPointDynamic("ope_matory")
    ...
    override fun stateChanged(state: BaseService.State, profileName: String?, msg: String?) {
        App.isConnectType = state.canStop
        if (App.isConnectType) {
            XLog.e("连接成功")
        }
        changeState(state)
    ...
    override fun onServiceConnected(service: IShadowsocksService) {
        ...
        App.isConnectType = BaseService.State.values()[service.state].canStop
        if (App.isConnectType) {
            model.mainCall?.changeOfVpnStatusCall(2)
            model.mainCall?.changeOfVpnStatusCall(2)
        } else {
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent?): Boolean {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            if (binding.viewGuideMask.isVisible) {
                ...
                if (!(!App.isConnectType && binding.vpnStateDynamic == 1)) {
                    finish()
                }
            }
        }
...
//        lifecycleScope.launch {
//            delay(300)
//            XLog.d("App.isConnectType=${App.isConnectType}")
//            if (!App.isConnectType) {
//                connect.launch(null)
//            }
//        }
